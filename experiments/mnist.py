...
for batch_idx, (data, target) in tqdm.tqdm(enumerate(train_loader)):
    if target[0] == 0:
        ...
        inferred_vars_list, inferred_coefs_list = hgm.infer_coefs(video, hgm.phis_list, max_itr=100, use_sparse=True)
        hgm.update_phis(video, inferred_coefs_list, use_warm_start_optimizer=(batch_idx > 0), use_sparse=True)

        torch.save(hgm.phis_list, "mnist0/phis_list_{}.torch".format(batch_idx))
        torch.save(inferred_coefs_list, "mnist0/inferred_coefs_list_{}.torch".format(batch_idx))